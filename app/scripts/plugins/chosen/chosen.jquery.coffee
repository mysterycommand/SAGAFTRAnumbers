...
define [
  'jquery'
  'plugins/chosen/lib/select-parser'
  'plugins/chosen/lib/abstract-chosen'
], ($, SelectParser, AbstractChosen) ->
  ...
  $.fn.extend
    chosen: (options) ->
      ...
      ###
        CHANGED July 28, 2012 by Matt Hayes
      ###
      return this if $('html').hasClass('lt-ie8')
  ...
  class Chosen extends AbstractChosen
    ...
    test_active_click: (evt) ->
      # log 'test_active_click', evt.target
      if $(evt.target).parents('#' + @container_id).length
        @active_field = true
      else
        ###
          CHANGED July 28, 2012 by Matt Hayes
          DO NOT HIDE THE MULTIPLE SELECT DROP DOWN WHEN THE USER DESELECTS!
          AN EXTRA CHECK HERE BECAUSE THE CHOICE IS REMOVED FROM THE DOM BEFORE THE CLICK EVENT IS TRIGGERED.
        ###
        # this.close_field()
        if $(evt.target).is '.search-choice-close'
          this.search_field_scale()
        else
          this.close_field()
    ...
    result_select: (evt) ->
      if @result_highlight
        ...
        ###
          CHANGED July 26, 2012 by Matt Hayes
          DO NOT HIDE THE MULTIPLE SELECT DROP DOWN WHEN THE USER SELECTS!
        ###
